...
def find_ytdlp() -> str | None:
    ...
    4. Our tools directory (~/.claudeclaw/tools/yt-dlp)
    """
    import shutil
    ...
    paths = [
        ...
        os.path.expanduser("~/.claudeclaw/tools/yt-dlp/yt-dlp"),  # Our tools
    ]
...
def convert_to_document(md_path: str, output_dir: str, doc_format: str, video_id: str = None) -> str:
    ...
    if doc_format == "md":
        ...
        pandoc_paths = [
            "pandoc",  # System PATH
            os.path.expanduser("~/.claudeclaw/tools/pandoc/pandoc"),  # Our tools
        ]
...
def find_mlx_audio(config: dict = None) -> tuple[str | None, str | None]:
    ...
    mlx_dir = os.path.expanduser(mlx_config.get("path", "~/.claudeclaw/tools/mlx-audio"))
...
def find_kokoro(use_cache: bool = True, config: dict = None) -> tuple[str | None, str | None]:
    ...
    2. Venv at ~/.claudeclaw/tools/kokoro/.venv (pip install)
    3. System Python with kokoro importable
    4. Legacy locations (cloned repos)
    """
    if config is None:
        config = load_config()
    kokoro_dir = os.path.expanduser("~/.claudeclaw/tools/kokoro")
    ...
    ml_env_python = os.path.expanduser("~/.claudeclaw/tools/ml-env/bin/python")
